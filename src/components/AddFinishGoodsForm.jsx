...
const componentSchema = z.object({
    product_id: z.string().regex(/^[A-Za-z0-9\s]+$/, "Product ID must only contain alphanumeric characters and spaces"),
    manufactured_date: z.date(), // Expect a valid JavaScript Date object
    manufactured_quantity: z.number().positive("Quantity must be a positive number"), // Expect a number
});
...
const FinishGood = ({ reFetchTableData }) => {
    ...
    const fetchProductDetails = async (product_id) => {
        try {
            ...
            setproduct_name("");
            setProductIdError("Product ID not found. Please check the ID and try again.");
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const payload = {
                ...
                manufactured_date: format(validateData.manufactured_date, "yyyy-MM-dd"), // Convert date to YYYY-MM-DD
                manufactured_quantity: validateData.manufactured_quantity.toString(), // Convert to string
            };
